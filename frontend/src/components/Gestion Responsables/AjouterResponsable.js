...
function AjouterResponsable() {
  ...
  const saveResponsable = (e) => {
    e.preventDefault();
    if (formIsValid) {
      const responsable = { cin, nom, prenom, dateNaissance, telephone, email, nationalite, ville, sexe, statut, login, password };
      responsableService.getResponsableById(cin).then((res) => {
        if (res.data && res.data.cin === cin) {
          toast.error(`La CIN ${cin} existe déjà dans la base de données.`, {
            position: toast.POSITION.TOP_CENTER,
          });
        } else {
          responsableService.createResponsable(responsable).then(() => {
              ...
              <div className="form-group row mt-4">
                <div className="col-sm-12 text-center">
                  <button
                    ...
                    aria-label="Enregistrer le responsable"
                  >
                    Enregistrer
                  </button>
                  <button
